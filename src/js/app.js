App = {
  ...
  searchAreaWise: async function () {
    try {
      ...
      const result = await App.instance.getAreaWiseCounts();
      const areas = result[0];
      const counts = result[1];
  
      // Combine and sort
      const areaCounts = areas.map((area, i) => ({ area, count: parseInt(counts[i]) }));
      areaCounts.sort((a, b) => b.count - a.count);
  
      // Display
      const tableBody = document.getElementById("areaCountTableBody");
      tableBody.innerHTML = "";
      areaCounts.forEach((row, index) => {
      ...
      console.error("Failed to load area-wise data:", err);
    }
  },
